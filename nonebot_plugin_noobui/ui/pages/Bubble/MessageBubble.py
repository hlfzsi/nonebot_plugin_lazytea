...
class NetworkManager(QObject):
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            cls._instance.manager = QNetworkAccessManager()
            ...
            cls._instance.manager.setCache(cache)
            cls._instance.manager.setCache(
                cls._instance.manager.cache())
        return cls._instance

    @property
    def qnam(self):
        """返回 QNetworkAccessManager 实例"""
        return self.manager
...
class MessageDetailDialog(QDialog):
    ...
    def _load_content(self, content: str, base_dir: str):
        ...
        self.scroll.setWidget(self.content_browser)
...
class MessageBubble(QFrame):
    ...
    def __init__(
        ...
        avatar_position: AvatarPosition = AvatarPosition.LEFT_OUTSIDE,
    ...
    def _initialize_properties(
        ...
        self.avatar_info: Optional[AvatarInfoType] = self.metadata.pop(
            "avatar", None)
        self.network_manager = NetworkManager().qnam
        self.pending_replies = []
        self.network_images = []
        ...
        def handle_avatar_reply():
            ...
            try:
                if reply.error() == QNetworkReply.NoError:
                    data = reply.readAll().data()  # 确保获取bytes数据
                    if not data:
    ...
    def _process_image_tags(self, soup: BeautifulSoup) -> None:
        ...
        for img in soup.find_all("img"):
            ...
            try:
                # 处理Base64编码的图片
                if re.match(r'^(?:data:image/[a-zA-Z]+;base64,|base64://)', src, re.IGNORECASE):
    ...
    def eventFilter(self, obj: QObject, event: QEvent) -> bool:
        if obj is self.content.viewport() and event.type() == QEvent.MouseButtonDblClick:
